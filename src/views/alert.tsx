...
function AlertPrimaryButton({kind, text, onClick}) {
  return (
    <button type="button" className="text-white bg-blue-800 hover:bg-blue-900 focus:ring-4 focus:outline-none focus:ring-blue-200 font-medium rounded-lg text-xs px-3 py-1.5 mr-2 text-center inline-flex items-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" onClick={onClick}>
      {text}
    </button>
  )
}
...
export function Alert({kind, text, title, primaryAction, primaryActionOnClick, secondaryAction=null, secondaryActionOnClick=null, className='', children}) {
  return (
    <div id="alert-additional-content-1" className={`p-4 mb-4 text-blue-800 border border-blue-300 rounded-lg bg-blue-50 dark:bg-gray-800 dark:text-blue-400 dark:border-blue-800 ${className}`} role="alert">
